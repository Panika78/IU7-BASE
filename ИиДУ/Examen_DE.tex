...
\subsection{Теорема Коши. Уравнения n-го порядка}

\examquestion{Сформулировать теорему Коши о существовании и единственности решения дифференциального уравнения $n$-го порядка. Интегрирование дифференциальных уравнений $n$-го порядка, допускающих понижение порядка.}
...
функция
\[
f(x,y(x),y'(x),\dots,y^{(n-1)}(x))
\]
и её частные производные
\[
\frac{\partial f}{\partial y},\quad \frac{\partial f}{\partial y'},\quad \dots,\quad \frac{\partial f}{\partial y^{(n-1)}}
\]
...
содержащей точку
\[
(x_0, y_0, y'_0, \dots, y^{(n-1)}_0),
\]
то существует и притом единственное решение
\[
y = \varphi(x, C_1, C_2, \dots, C_n),
\]
дифференциального уравнения $n$-го порядка, удовлетворяющее начальному условию.
}

\examanswer{

\examstep{1. Уравнения вида $y^{(n)} = f(x)$}

Метод решения: $n$-кратное интегрирование
\[
y = \int \dots \int f(x)\,(dx)^n.
\]

\examstep{2. Уравнения, не содержащие переменную $x$ явно}
...
\qquad \dots,
\qquad
y^{(n)} = p^{(n-1)} \cdot \dots \cdot p.
\]

\examstep{3. Уравнения, не содержащие функцию $y$ в явном виде}

\[
F(x, y', \dots, y^{(n)}) = 0.
\]

Метод решения: понижение порядка при помощи замены
\[
y' = p(x),
\qquad
y'' = p'(x),
